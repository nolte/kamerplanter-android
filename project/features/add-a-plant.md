...
- **acceptance-8** — `InstanceIdProposalTest` (six cases held against the web UI's `idGenerator.ts`: shape, sanitising, the `PLANT` placeholder, date, suffix wrap, the step past a taken identifier), `PlantCaptureViewModelTest` `the identifier is proposed from the species in the web UI's shape and skips what is taken`, `a numeric species key still yields a proposal, from the scientific name`, `an identifier the user edited is never overwritten by a proposal`, `a submission without species, identifier or date is refused field by field`, `a taken identifier is refused before anything is sent` — **met 2026-09-03** (the 2026-09-02 proposal was empty on the device: it derived its prefix from numeric species keys — see the correction record in `project/requirements/plant-capture.md`)
