...
- **acceptance-2** — the picker offers the microscope only while a device is attached, and disabled rather than hidden so its absence reads as "not plugged in"; screen-level, no assertion yet — **met 2026-08-28** — `CaptureActions.kt:65,125` gates the microscope action on `microscopeAttached`; `CanFireTest` `the microscope fires only while it is streaming`
...
- **acceptance-5** — the shutter is gated on `Ready.canFire`, which requires a streaming device for the microscope source; screen-level, no assertion yet — **met 2026-08-28** — `CaptureActions.kt:190` maps every `UnavailableReason` to its own wording and disables the action; `CanFireTest` `no source means no shutter`
