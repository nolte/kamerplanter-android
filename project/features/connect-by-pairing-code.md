...
- [x] **acceptance-6** A locked-out redemption states how long the lockout lasts, and a rate-limited one is distinguishable from it.
...
- **acceptance-6** — unit test over the `423` and `429` mappings — **met 2026-09-01** — `NetworkConnectionClientTest`: `a locked-out address is not reported as an expired code` and `a rate-limited pairing attempt is not reported as an expired code` keep the two apart; `a lockout names its duration when the instance sends one` and `a long lockout is described in minutes, rounded up` surface the `Retry-After` value. Where the instance sends no `Retry-After`, no duration is invented — the wording falls back to "wait before trying again", per the issue-#53 rule against guessed numbers
