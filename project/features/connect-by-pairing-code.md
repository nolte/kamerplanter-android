...
- **acceptance-4** — **half regression, half new.** The adoption rule ships and is tested (`SettingsViewModel.kt:173-187`); the tenant-picker UI does not exist — `SelectingTenant` currently renders the same spinner as `Verifying` — **met 2026-08-28** — `SettingsViewModelTest`: `a valid qr connects and persists the connection` (single tenant adopted straight through), `several tenants pause the flow until the user picks one`, `a tenant that was never offered is not adopted`; the picker ships as `TenantChoiceBody` (`SettingsScreen.kt:183`) wired to `selectTenant` (`SettingsViewModel.kt:240-248`). The note above claiming the picker is missing was stale
